...
export default function HeroServicesPhoneCarry() {
  const [carry, setCarry] = useState<CarryState>({ x: 0, y: 0, scale: 0.96, opacity: 0 });
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      const startX = window.innerWidth < 640 ? -window.innerWidth * 0.44 : -window.innerWidth * 0.38;
      const centerX = 0;
      ...
      const stage1 = clamp01((scrollY - stage1Start) / Math.max(1, stage1End - stage1Start));
      ...
      const opacity = clamp01((scrollY - appearStart) / Math.max(1, appearEnd - appearStart));
  ...
  return (
    <motion.div
      ...
      animate={{ x: carry.x, y: carry.y, scale: carry.scale, opacity: carry.opacity }}
    ...
    >
      <div className="relative h-[300px] w-[150px] rounded-[36px] border-[3px] border-slate-500/90 bg-black p-[6px] shadow-[0_24px_60px_rgba(0,0,0,0.45)] sm:h-[560px] sm:w-[280px] sm:rounded-[56px] sm:p-[10px]">
        ...
        <div className="flex h-full w-full items-center justify-center rounded-[30px] bg-gradient-to-b from-sky-100 to-sky-200 sm:rounded-[46px]">
          <Image src="/favicon-clean.png" alt="Tappit splash logo" width={72} height={72} className="drop-shadow-lg sm:h-[98px] sm:w-[98px]" />
        </div>
      </div>
    </motion.div>
  );

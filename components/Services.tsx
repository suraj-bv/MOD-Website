...
function ServiceCard({ name, image, emoji, index }: Service & { index: number }) {
  return (
    <article className="w-[128px] overflow-hidden rounded-3xl border border-white/70 bg-white shadow-lg shadow-sky-800/20 sm:w-[192px]">
      ...
      <p className="truncate px-2 py-3 text-center text-xs font-semibold text-brand-dark sm:px-3 sm:text-base">{name}</p>
    </article>
  );
}
  ...
  useEffect(() => {
    const onScroll = () => {
      ...
      if (rect.bottom > hideAt) {
        setShowInlineDownload(true);
        const progress = (rect.bottom - hideAt) / Math.max(1, fadeStart - hideAt);
      ...
      <div className="relative mx-auto w-full max-w-7xl px-4 sm:px-8">
        <div className="text-center">
          ...
          <p className="mx-auto mt-3 max-w-2xl text-sm text-sky-100 sm:text-base">
            Pick a service and get instant slots. Fast, verified and always on time.
          </p>
        </div>
        ...
        <div className="relative left-1/2 mt-12 h-[820px] w-screen -translate-x-1/2 sm:h-[570px]">
            ...
            <div className="marquee-container overflow-hidden">
              <div className="marquee-track" style={marqueeStyle}>
                {track.map((service, index) => (
                  <div key={`${service.name}-${index}`} className="mx-2.5 sm:mx-3.5">
          ...
          <motion.div style={{ y: phoneY, scale: phoneScale }} className="absolute left-1/2 top-1/2 z-30 -translate-x-1/2 -translate-y-1/2">
              ...
              <div className="w-[205px] rounded-[2.8rem] border-[7px] border-slate-800 bg-black p-0 shadow-xl shadow-sky-800/25 sm:w-[260px]">
                <div className="relative h-[360px] overflow-hidden rounded-[2.2rem] bg-white sm:h-[440px]">
                  ...
                  <div className="absolute inset-0 flex flex-col items-center justify-center">
                    <Image src="/favicon0clean.png" alt="Tappit splash" width={86} height={86} className="drop-shadow-xl" />
                    <p className="font-display mt-5 text-5xl font-black text-sky-600">Tappit</p>
                    <p className="text-sm text-slate-500">Spotless in Minutes</p>
                  </div>
                </div>
              </div>
            </motion.div>
          ...
          {showInlineDownload ? <div className="absolute -bottom-12 left-2 z-40 transition-opacity duration-150 sm:-bottom-8 sm:left-4" style={{ opacity: inlineCardOpacity }}>
            <div className="rounded-3xl border border-white/60 bg-white/95 p-4 shadow-xl shadow-sky-800/18 backdrop-blur-sm sm:p-6">
              ...
              <p className="text-3xl font-black text-sky-600">The Tappit</p>
